    ...
    <el-tabs v-model="activeName">
      <el-tab-pane label="应用插件" name="first">
        <el-row class="henglan">
          <nuxt-link @click="xianshi = false" v-for="item in option" :key="item.id" :to="item.status === 1 ? '/manage/' + (item.name_pinyin === 'canzhuoguanli' ? 'shopManage' : item.name_pinyin) + '?plugin_id=' + item.id + (item.name_pinyin === 'canzhuoguanli' ? '&id=2' : '') : ''">
